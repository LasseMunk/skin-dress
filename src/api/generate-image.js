...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.debug("[api/generate-image] Payload", { hasPrompt: !!prompt, hasPerson: !!person_image, clothingCount: Array.isArray(clothing_images) ? clothing_images.length : 0, output_format });

    if (!person_image || !Array.isArray(clothing_images) || clothing_images.length < 1) {
      return res.status(400).json({ error: "person_image and at least one clothing_images item are required" });
    }
    ...
    console.debug(
      ...
      Buffer.isBuffer(personInput) ? `Person(Buffer ${personInput.length})` : `Person(${String(personInput).slice(0,64)})`,
